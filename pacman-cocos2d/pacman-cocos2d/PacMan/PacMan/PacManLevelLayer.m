...
@interface PacManLevelLayer ()
{
    ...
    int  _coinsCount;
    CGPoint _spawnPoint;
    PacManGameObject *_player;
    CCSpriteBatchNode *_buttonSprites;
    TouchPad *_touchPad;
}

@property (nonatomic, strong) PacManGameObject *player;
...
-(id) init
{
	if( (self=[super init]) ) {
        ...
        _player = [[PacManGameObject alloc] init];
        ...
        [_touchPad touchButton:TouchPadButtonRight];
...
-(void)touchPad:(TouchPad *)touchPad didTouchButton:(TouchPadButton)button{
    switch (button) {
        case TouchPadButtonDown:{
            [_player setFutureDirection:NavigationDirectionDown];
            break;
        }
        case TouchPadButtonUp:{
            [_player setFutureDirection:NavigationDirectionUp];
            break;
        }
        case TouchPadButtonLeft:{
            [_player setFutureDirection:NavigationDirectionLeft];
            break;
        }
        case TouchPadButtonRight:{
            [_player setFutureDirection:NavigationDirectionRight];
            break;
        }
        default:
            break;
...
-(void) update:(ccTime)delta
{
    ...
    //Checks for Ghosts
    for (GhostGameObject *ghost in self.ghosts)
    {
...
-(void)resumeGame
{
    ...
    for (GhostGameObject *ghost in _ghosts){
        [self removeChild:ghost];
    }
    [self loadGhosts];
    [_ghosts makeObjectsPerformSelector:@selector(startNavigating)];
...
-(void) loadGhosts
{
    ...
    for (int i = 0; i< kGhostCount; i++){
        GhostGameObject *ghost = [self spawnGhost];
        [_ghosts addObject:ghost];
    }
}

-(GhostGameObject *) spawnGhost
{
    GhostGameObject *ghost = [[GhostGameObject alloc] init];
    if (ghost){
        [self addChild:ghost];
        [ghost setSpeed:80.0];
